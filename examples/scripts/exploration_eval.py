...
if __name__ == "__main__":
    ...
    logs      = envs.logged(CycledLearner(cb.VowpalEpsilonLearner()))
    logs_take = envs_take.logged(CycledLearner(cb.VowpalEpsilonLearner()))
    ...
    print(f"{datetime.datetime.now()} off-policy")
    r = cb.Experiment(logs_take.ope_rewards("DM"), cb.VowpalEpsilonLearner(), evaluation_task=cb.OffPolicyEvaluation()).run(processes=n_processes,quiet=True)
    ...
    r = cb.Experiment(logs_take.ope_rewards("DM"), cb.VowpalEpsilonLearner(), evaluation_task=cb.OnPolicyEvaluation()).run(processes=n_processes,quiet=True)
    ...
    for i,(c,q,ope) in enumerate(product([None],[.005,.002],[False,True])):
        print(f"{datetime.datetime.now()} explore-eval {i}")
        l = logs.ope_rewards('DM') if ope else logs
        r = cb.Experiment(l, cb.VowpalEpsilonLearner(), evaluation_task=cb.ExplorationEvaluation(ope=ope, cinit=c, qpct=q, cmax=c or 1)).run(processes=n_processes, quiet=True)
        r.plot_learners(labels=f'c={c},q={q},ope={ope}',colors=(i+3),out=None)
